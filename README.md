...
- 국내 주식 평단가는 원화, 미국 주식 평단가는 달러로 입력
- 종목별·섹터별·국내외 투자 비중 차트
- 미국 종목 평가금액을 최신 원/달러 환율로 환산한 원화 기준 자산 비중
...
`buy_price`는 국내 종목(`.KS`, `.KQ`)이면 원화, 미국 종목이면 달러로 저장합니다. 대시보드의 총 투자금액과 원형 그래프는 미국 종목 금액에 최신 원/달러 환율을 적용해 모두 원화 기준으로 계산합니다.
